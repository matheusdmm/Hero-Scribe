...
describe('emptyCharacter', () => {
  it('languages starts empty — Common is added by ExtrasStep on mount', () => {
    expect(emptyCharacter().languages).toEqual([])
  })

  it('has all required draft fields', () => {
    const c = emptyCharacter()
    for (const field of ['name', 'race', 'class', 'background', 'edition', 'alignment',
                          'skills', 'spells', 'equipment', 'weapons', 'languages']) {
...
describe('Race languages', () => {
  ...
  it('dwarf has Dwarvish', () => expect(RACE_LANGUAGES.dwarf).toContain('Dwarvish'))
  it('tiefling has Infernal', () => expect(RACE_LANGUAGES.tiefling).toContain('Infernal'))
  it('dragonborn has Draconic', () => expect(RACE_LANGUAGES.dragonborn).toContain('Draconic'))
})
...
describe('Weapon proficiency', () => {
  ...
  it('simple proficiency grants dagger',           () => expect(isProficientWith(dagger,    ['simple'])).toBe(true))
  it('simple proficiency grants shortbow',         () => expect(isProficientWith(shortbow,  ['simple'])).toBe(true))
  it('simple proficiency does not grant longsword',() => expect(isProficientWith(longsword, ['simple'])).toBe(false))
  it('martial proficiency grants longsword',       () => expect(isProficientWith(longsword, ['martial'])).toBe(true))
  it('empty proficiency denies all',               () => expect(isProficientWith(dagger,    [])).toBe(false))
})
...
describe('SPELLCASTING_CLASSES set', () => {
  ...
  for (const cls of casters)
    it(`${cls} is a spellcaster`, () => expect(SPELLCASTING_CLASSES.has(cls)).toBe(true))

  for (const cls of noncasters)
    it(`${cls} is not a spellcaster`, () => expect(SPELLCASTING_CLASSES.has(cls)).toBe(false))
})
...
describe('Go API — needs vercel dev on :3000', () => {
  it('GET /api/races returns 8 races with required fields', async () => {
    ...
    for (const r of races)
      expect(r).toMatchObject({ id: expect.any(String), name: expect.any(String), speed: expect.any(Number) })
  })
  ...
  it('POST /api/calculate returns a valid sheet', async () => {
    const res = await fetch(`${LOCAL_API}/calculate`, {
      ...
      body: JSON.stringify({
        race: 'elf', class: 'wizard', level: 5, background: 'Sage',
        abilities: { strength: 8, dexterity: 15, constitution: 14, intelligence: 17, wisdom: 12, charisma: 10 },
      }),
    })
    expect(res.ok).toBe(true)
  ...
  it('POST /api/calculate HP formula is correct (elf wizard L5)', async () => {
    ...
    const res   = await fetch(`${LOCAL_API}/calculate`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        race: 'human', class: 'wizard', level: 5, background: 'Sage',
        abilities: { strength: 8, dexterity: 10, constitution: 14, intelligence: 17, wisdom: 12, charisma: 10 },
      }),
    })
    const sheet = await res.json()
...
describe('Open5e — spell_lists filter', () => {
  // paladin + ranger use dnd_class fallback (not valid spell_lists choices in Open5e)
  const SPELL_LIST_CLASSES = ['bard', 'cleric', 'druid', 'sorcerer', 'warlock', 'wizard']
  const DND_CLASS_CLASSES   = ['paladin', 'ranger']
  ...
  for (const cls of DND_CLASS_CLASSES) {
    it(`${cls} returns spells via dnd_class fallback`, async () => {
      const name = cls.charAt(0).toUpperCase() + cls.slice(1)
      const res  = await fetch(`${OPEN5E}/spells/?dnd_class=${name}&limit=1`)
      const data = await res.json()
      expect(data.count, `${cls} returned 0 spells`).toBeGreaterThan(0)
    }, 15000)
  }
...
describe('Extended items — WotC raw data', () => {
  ...
  beforeAll(async () => { items = await getExtendedItems() }, 60000)
